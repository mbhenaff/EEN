...
class ImageLoader(object):
    def __init__(self, arg):
        ...
        # train event dataset XXX: toooooo ugly
        if not arg.has_key("train_event_datapath"):
            arg["train_event_datapath"] = os.path.join(os.path.dirname(arg.get("datapath")),
                                                "train_event_data")
        self.e_train_datalist = []
        self.e_train_event_datalist = []
        for fdname in os.listdir(arg.get("train_event_datapath")):
            fd_datalist = []
            #print("loading {}".format(fdname))
            abs_fdname = os.path.join(arg.get("train_event_datapath"), fdname)
            presaved_npy = glob.glob(os.path.join(abs_fdname, "*.npy"))
            if len(presaved_npy) == 1:
                fd_datalist = numpy.load(presaved_npy[0])
            elif len(presaved_npy) == 0:
                ...
                raise ValueError
            self.e_train_datalist.append(fd_datalist)
            self.e_train_event_datalist.append(self._parse_file(os.path.join(abs_fdname, "events")))
        self.e_train_event_datalist_full = []
        for elem, elem_state in zip(self.e_train_event_datalist, self.e_train_datalist):
            # each elem is one episode
            elem_event = []
            for id in range(len(elem_state)):
                this_key = "{:06d}".format(id)
                if elem.has_key(this_key):
                    this_val = elem.get(this_key)
                    if this_val[0] == 32 and this_val[1] == 2:
                        elem_event.append(1)
                    else:
                        elem_event.append(0)
                else:
                    elem_event.append(0)
            self.e_train_event_datalist_full.append(elem_event)
        self.e_train_event_datalist = self.e_train_event_datalist_full
        
        # test event dataset
        if not arg.has_key("test_datapath"):
            arg["test_datapath"] = os.path.join(os.path.dirname(arg.get("datapath")),
                                                "testdata")
        self.e_test_datalist = []
        self.e_test_event_datalist = []
        for fdname in os.listdir(arg.get("test_datapath")):
            fd_datalist = []
            #print("loading {}".format(fdname))
            abs_fdname = os.path.join(arg.get("test_datapath"), fdname)
            presaved_npy = glob.glob(os.path.join(abs_fdname, "*.npy"))
            if len(presaved_npy) == 1:
                fd_datalist = numpy.load(presaved_npy[0])
            elif len(presaved_npy) == 0:
                ...
                raise ValueError
            self.e_test_datalist.append(fd_datalist)
            self.e_test_event_datalist.append(self._parse_file(os.path.join(abs_fdname, "events")))

        self.e_test_event_datalist_full = []
        for elem, elem_state in zip(self.e_test_event_datalist, self.e_test_datalist):
            # each elem is one episode
            elem_event = []
            for id in range(len(elem_state)):
                this_key = "{:06d}".format(id)
                if elem.has_key(this_key):
                    ...
                    elem_event.append(0)
            self.e_test_event_datalist_full.append(elem_event)
        self.e_test_event_datalist = self.e_test_event_datalist_full
